...
function JournalIntro({
  ...
  if (richText?.length) {
    return (
      <RichText
        value={richText}
        className="mb-10 w-full text-justify text-xs font-light leading-[1.9] text-ink"
      />
    );
  }
  const lines = paragraphs?.length ? paragraphs : FALLBACK_INTRO;
  return (
    <div className="mb-10 w-full space-y-4 text-justify text-xs font-light leading-[1.9] text-ink">
      {lines.map((line) => (
        <p key={line}>{line}</p>
      ))}
    </div>
    ...
    >
      ...
      {(entry.blurbRich || entry.blurb || entry.noteRich || entry.note) && (
        <motion.div
          ...
          className="mt-8 w-full space-y-4 text-justify text-xs font-light leading-[1.9] text-ink"
        >
          <RichText
